...
import { postAddToPractitionerGallery } from "../../../redux/slices/practitioner/practitioner_gallery";
...
const CreateAccountPractitioner = () => {
  ...
  const handleSubmit = async () => {
    ...
    const body: PractitionerRegisterData = {
      ...practitioner,
      hourly_rate: Number(practitioner.hourly_rate),
      address: {
    ...
    await handleSubmitGallery(token);
  ...
  const handleSubmitGallery = async (token: string) => {
    const body = new FormData();
    ...
    const response = await dispatch(
      postAddToPractitionerGallery({ body, token })
    );
    if (response.error) {
      setSubmitting(false);
      messageAlert(
